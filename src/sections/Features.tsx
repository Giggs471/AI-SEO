...
import  {animate, motion, useMotionTemplate, useMotionValue, useReducedMotionConfig, ValueAnimationTransition } from "framer-motion"
...
const FeatureTab = (props: typeof tabs[number] & ComponentPropsWithoutRef<'div'> & {selected: boolean}) => {
  ...
  return(
    <div ref={tabRef} onClick={props.onClick}
    onMouseEnter={handleTabHover} className="border border-white/15 flex p-2.5 rounded-xl gap-2.5 items-center lg:flex-1 relative">
      {props.selected && (
        <motion.div 
        style={{
          maskImage,
        ...
        className="absolute inset-0 -m-px rounded-xl border border-[#a369ff] "></motion.div>
          ...
          <div className="h-12 w-12 border border-white/15 rounded-lg inline-flex items-center justify-center">
            <DotLottiePlayer src={props.icon} className="h-10 w-10" autoplay  ref={dotLottieRef}/>
          </div>
          <div className="font-medium">{props.title}</div>
          {props.isNew && <div className="text-xs rounded-full px-2 py-0.5 bg-[#8c44ff] text-white brightness-125 font-semibold">new</div>}
        </div>
  )
}
...
export const Features = () => {
  ...
  return <section className="py-20 md:py-24">
    <div className="container">
      ...
      <p className="text-white/70 text-lg tracking-tighter text-center mt-5 md:text-xl max-w-2xl mx-auto">From small startups to large enterprises, our AI-driven tool has revolutionized the way businesses approach SEO.</p>
